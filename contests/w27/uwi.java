...
public class D {
	...
	void solve()
	{
		...
		for(int i = 0;i < n;i++){
			roots[s[i]-'a'] = merge(roots[s[i]-'a'], new Node(i));
		}
		...
		for(int z = 0;z < Q;z++){
			...
			if(t == 1){
				...
				turn += 26;
				for(int i = 0;i < 26;i++){
					int rpos = lowerBound(roots[i], r+1);
					int lpos = lowerBound(roots[i], l);
					Node[] lm_r = split(roots[i], rpos);
					Node[] l_m = split(lm_r[0], lpos);
					ls[i] = l_m[0];
					ms[i] = l_m[1];
					rs[i] = lm_r[1];
				}
				for(int i = 0, j = turn;i < 26;i++,j++){
					if(j == 26)j = 0;
					roots[i] = merge(merge(ls[i], ms[j]), rs[i]);
				}
			}else{
				int l = ni(), r = ni();
				int[] nums = new int[26];
