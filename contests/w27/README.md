...
My answer: Jan-16
```
그리고 제가 가끔 방문하는 일본 여자 사이트인데 ..이 개발자 분도 (노영만 님이 년말에 많은 시간을 들여서 공부하였다는.. ) 지난 27주차 마지막 문제를 가지고 본인이 공부하고 정리를 해서 올려놓았더군요..

http://pekempey.hatenablog.com/entry/2016/12/30/144923

이 여자 분 사이트에는 재미있게 두 가지 사이트가 연결되어 있는데 하나는 Uwi 이고 다른 하나는 러시아 인 개발자 사이트입니다. 이 일본 개발자 분도 이 문제를 이해하려고 결사적으로 노력을 한 것을 느꼈습니다.

https://www37.atwiki.jp/uwicoder/pages/2842.html (Uwi)

http://e-maxx.ru/algo/suffix_automata (러시아인)
```
3rd comment

```
일단은 suffix array로 푸는 해법을 찾기 위해서 고군분투하다보니 link-cut tree난 suffix automation은
조금 미뤄뒀습니다.
suffix automation 코드는 만들어는 놓았지만 아직 연구를 본격적으로 시작은 하지 않았습니다.
그 여자분 사이트도 잘 정리해 놓았더라구요.
저도 시간을 내서 꼭 다시 볼 예정입니다.

그보다 먼저 저는 WoC 27일차 editorial에 있는 tester's code를 정말 유심히 보았습니다.
이 코드는 suffix array로만 문제를 풀었더군요.
이 코드를 제 나름대로 해석하고 정리해서 sparse table에 대해서도 공부하고
Suffix array, suffix LCP 등에 대해서 더 깊이있게 이해하게 되었습니다.
다른 참가자들의 코드에서 보였던 이해안가던 부분들이 이 코드를 분석하면서 많이 이해하게 되었어요.
```
---
